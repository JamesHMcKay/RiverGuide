...
class FlowChart extends Component<IFlowChartProps, IFlowChartState> {
  ...
  public getObservables = (): IObservable[] | undefined => {
    const observables: IObservable[] | undefined =
      this.props.infoPage.selectedGuide.observables;
    if (observables) {
      return observables;
    } else {
  ...
  public mapHistory = (history: IHistoryNew): IChartData[] => {
    const type: string = this.state.selectedType || "flow";
    const key: keyof IHistoryNew = type as keyof IHistoryNew;
    if (!history[key]) {
      return [];
    }

    const result: IChartData[] = history[key].map(
      (reading: IObservation): IChartData => {
        return {
  ...
  public setChartOptions = (): void => {
    ...
    const series: Am4charts.ColumnSeries | Am4charts.LineSeries =
      this.state.selectedType === "rainfall"
  ...
  public componentDidUpdate(nextProps: IFlowChartProps): void {
    if (this.props.expansionPanels.flowHistory) {
      if (
        this.props.infoPage.selectedGuide.id !==
        nextProps.infoPage.selectedGuide.id
      ) {
        let selectedType: string | undefined;
        const observables: IObservable[] | undefined = this.getObservables();
        if (observables) {
  ...
  public historySame = (
    ...
    for (const _key of Object.keys(listOne)) {
      ...
      if (valuesListTwo === null && !!valuesListOne) {
        return false;
      }
      if (valuesListOne === null && valuesListTwo === null) {
        return true;
      }

      if (valuesListOne.length !== valuesListTwo.length) {
        return false;
      }
  ...
  public shouldComponentUpdate = (nextProps: IFlowChartProps): boolean => {
    ...
    if (
      !this.historySame(this.props.infoPage.history, nextProps.infoPage.history)
    ) {
      return true;
    }
    ...
    return (
      <div>
        ...
        >
          ...
          {visible && (
            <div className="flow-chart-buttons">{this.getButtons()}</div>
          )}
        </div>
        {visible && (
