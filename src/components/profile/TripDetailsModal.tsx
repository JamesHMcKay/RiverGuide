...
class TripDetailsModal extends Component<
  ITripDetailsModelProps,
  ITripDetailsModelState
> {
  constructor(props: ITripDetailsModelProps) {
    ...
    if (this.props.initialLogEntry) {
      ...
      const guideId: string | undefined = this.props.selectedGuide
        ? this.props.selectedGuide.id
        : "";
  ...
  public handleSave = (): void => {
    ...
    logEntry = {
      ...
      start_date_time: this.state.start_date
        ? this.state.start_date.toISOString()
        : "",
      end_date_time: this.state.end_date
    ...
    if (logEntry.guide_id === "") {
      this.setState({
        error:
          "Select a section - if your section is not available let us know",
      });
      return;
    }
  ...
  public getPersonList(): JSX.Element {
    ...
    listItems.push(
      <li key={keys.length + 1} style={{ float: "left" }}>
        <div>
          <span className="person-icon" onClick={this.addPerson.bind(this)}>
        ...
        <DialogContent>
          ...
          {this.state.peopleCount > 4 && (
            <TextField
              ...
              onChange={(value: any): void =>
                this.setState({ peopleCount: value.target.value })
              }
              fullWidth
            />
          )}
          <div className="date-picker-container">
            <MuiPickersUtilsProvider utils={DateFnsUtils}>
              ...
              <TimeSlider
                handleChange={this.handleTimeRangeChange}
          ...
          >
            <Button
              ...
              onClick={(event: any): void =>
                this.handlePublicChange(event, "public")
              }
            >
              {"Public"}
            </Button>
            <Button
              ...
              onClick={(event: any): void =>
                this.handlePublicChange(event, "private")
              }
            >
              {"Private"}
            </Button>
